% Stam et al.'s model with synaptic scaling, where we can change a_ss
function [C_t, E_t] = NMMStamStability(a_ss)
  %% scripts
  ...
  % plasticity
  a_sdp = 0.01; % SDP step size, 0 means no SDP
  b_sdp = 2;
  n_d = 20; % delay window - used for SDP
  t_p = 20; % plasticity every 20 steps
  a_gdp = 0.001; % GDP step size, 0 means no GDP
  c_gdp = 0.2;
